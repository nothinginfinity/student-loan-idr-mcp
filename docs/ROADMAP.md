...
1. **One comparison authority.** Reuse the existing owner-scoped deterministic comparison endpoint/snapshot and FLRs tie semantics. PDF/image/report rendering must consume that immutable normalized comparison result; it must not recompute repayment formulas in a separate renderer.
2. **Professional report artifact.** Add a borrower-facing comparison report that can include borrower/advisor display identity, calculation date, policy snapshot, calculation/snapshot identifier, current modeled monthly payment, modeled total borrower paid, supported remaining-balance/forgiveness outputs, balance/payment path visualizations, and the existing assumptions/non-guarantee language. Long-term values that the deterministic engine withholds must remain withheld in the artifact.
3. **Portable downloads.** Provide an advisor-controlled first-class PDF/print artifact plus a secondary image/SVG export suitable for text/email presentation. Exports must be generated from the same comparison snapshot, use no third-party tracking/chart runtime, escape user-controlled content, and remain usable without weakening the existing no-network document CSP/privacy posture.
4. **Secure delivery.** Preserve the current high-entropy opaque share-token model, frozen comparison snapshot, exact-owner issuance/revocation, bounded borrower review/signing window, and timeline/audit behavior. Add clear advisor controls to view the interactive comparison, download the artifact, create/revoke a secure link, and copy/send the same frozen comparison rather than creating divergent versions.
5. **Privacy and traceability.** Do not place raw StudentAid text, evidence files, SSNs, FSA credentials, raw award IDs, or unrelated client facts in the artifact or share payload. Every exported/shared report should carry enough non-secret snapshot identity and as-of/policy metadata for the advisor to reproduce which deterministic comparison was shown.
6. **Acceptance.** Prove exact cross-advisor isolation, FLRs winner/tie behavior, immutable snapshot parity between interactive/share/export surfaces, safe escaping, no external assets/analytics, revocation/expiration behavior, no raw-FSA retention, timeline traceability, and exact-SHA production deployment before closure.
...
1. **Real evidence-grounded synthesis.** Replace the current deterministic summary-only final answer layer with a provider-neutral LLM synthesis step over the bounded V0.9.7 evidence packet. The model may explain, compare, summarize, ask a follow-up question, and propose explicit next actions, but it must not perform authoritative repayment math, invent eligibility, or silently mutate client facts.
2. **Conversation quality.** Support natural reformulations and bounded multi-turn advisor/borrower follow-up while preserving exact client isolation, borrower-safe context narrowing, citation/provenance display, and explicit missing/ambiguous-fact handling. Conversation state must not become a hidden source of authoritative case facts.
3. **Versioned Knowledge Packs.** Move reviewed explanatory knowledge into versioned packs separated by authority tier, at minimum **official federal authority** and **accepted specialty/advisor knowledge**. Each item carries stable ID, title/content, authority tier, source/provenance, effective range, reviewed date, content hash, relevant programs/loan families, keywords/tags, and allowed uses.
4. **Official vs specialty boundary.** Official sources may support policy explanations; accepted specialty knowledge may support advisor workflow, operational observations, and consultative phrasing. Specialty notes must never masquerade as federal authority or override deterministic formula/eligibility code. Any specialty rule intended to change math/eligibility must be promoted separately into reviewed deterministic code with source-backed tests.
5. **Retrieval expansion.** Expand the current small reviewed policy corpus using exact-keyword plus semantic/hybrid retrieval where appropriate, while keeping normalized client PII in owner-scoped structured retrieval rather than a shared vector corpus. Raw StudentAid embeddings remain prohibited.
6. **Model fail-closed behavior.** If synthesis/model routing fails, stale policy is detected, citations cannot be validated, or evidence is insufficient, return a deterministic bounded fallback rather than an uncited confident answer.
