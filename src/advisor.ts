...
export async function handleAdvisorApi(request: Request, env: AdvisorWorkspaceEnv): Promise<Response> {
  ...
  try {
    ...
    if(request.method==="GET"&&u.pathname==="/api/advisor/session"){const a=await authenticate(request,database,false);return json({ok:true,advisor:accountView(a.account),expiresAt:a.session.expires_at});}
