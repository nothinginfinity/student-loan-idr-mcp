...
function clientFromSnapshotBasis(row: SnapshotRow, basis: JsonObject): AdvisorClientRecordV1 {
  return { schemaVersion:1, clientId:row.client_id, ownerAdvisorId:row.owner_advisor_id, createdAt:row.created_at, updatedAt:row.created_at, lifecycleState:"active", readinessState:"needs_evidence", contact:{displayName:"Retained snapshot basis"}, ...(basis.confirmedFacts?{confirmedFacts:basis.confirmedFacts as AdvisorClientRecordV1["confirmedFacts"]}:{}), ...(basis.normalizedLoanPortfolio?{normalizedLoanPortfolio:basis.normalizedLoanPortfolio as AdvisorClientRecordV1["normalizedLoanPortfolio"]}:{}), ...(Array.isArray(basis.consideredPlans)?{consideredPlans:basis.consideredPlans as RepaymentPlan[]}:{}) };
}
