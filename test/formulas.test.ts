...
test("borrower UI serves a privacy-safe same-origin calculator shell", async () => {
  ...
  assert.match(html, /Parser diagnostics/);
  assert.match(html, /Unmapped FSA labels/);
  assert.match(html, /Imported from StudentAid · Delinquency/);
...
test("MCP initialize negotiates the declared protocol revision and current server version", async () => {
  ...
  assert.equal(body.result.serverInfo.version, "0.9.7");
});
